...
plot_mds_mle = function(obj, asp = TRUE, ncores = parallel::detectCores(),
                        nsubsample = 100, cor_scaling_factor = 1) {
  ...
  sample_condition_donor = function(k, tb_info) {
    set.seed(seed)
    # fixed effects
    beta = stan_pars$beta[,tb_info$term_index]
    # cell random effect
    if(tb_info$term_index == 1) {
      sigma = stan_pars$sigma
      Q = stan_pars$Q
    } else {
      sigma = stan_pars$sigma_term
      Q = stan_pars$Q_term
    }
    Cov = Q %*% diag(sigma^2) %*% t(Q)
    # donor random effect
    b_donor = stan_pars$b_donor[tb_info$donor_index,]
    # combine
  ...
  set.seed(seed)
  expr_median = mclapply(1:nsubsample,
                         function(i) {
                           sample_mu_hat(k = i) %>%
