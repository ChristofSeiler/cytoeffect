...
* Subset A: Check if we can model pSTAT, pSTAT3, and pSTAT5 bright cells
* Subset B: Check if we can model pSTAT bright, and pSTAT3 and pSTAT5 dim cells
...
gof = function(tb) {
  tb %>% 
    group_by(term) %>% 
    summarize(
      subset_A = mean(pSTAT1 > pSTAT1_median & pSTAT3 > pSTAT3_median & pSTAT5 > pSTAT5_median),
      subset_B = mean(pSTAT1 > pSTAT1_median & pSTAT3 < pSTAT3_median & pSTAT5 < pSTAT5_median),
      subset_C = mean(pERK1_2  == 0),
      subset_D = mean(pMAPKAPK > pMAPKAPK_median)
      )
}
...
gof_pred %<>% gather(subset, statistic, -term)
gof_obsv %<>% gather(subset, statistic, -term)
