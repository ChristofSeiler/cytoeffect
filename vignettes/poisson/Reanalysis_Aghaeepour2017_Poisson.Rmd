...
# traceplot for covariance from orthogonal matrix Q
stan_pars = rstan::extract(obj$fit_mcmc, 
                           pars = c("sigma","sigma_term","sigma_donor",
                                    "Q","Q_term","Q_donor"))
cors = lapply(1:nrow(pars$Q), function(k) {
  sigma = stan_pars$sigma_donor[k,]
  Q = stan_pars$Q_donor[k,,]
  cor = cov2cor(Q %*% diag(sigma^2) %*% t(Q))
  cor
})
cors_arr = simplify2array(cors)
tb = tibble(
  chain = rep(paste(1:8), each = 125),
  k = 1:dim(cors_arr)[3],
  cor = cors_arr[3,6,]
  )
ggplot(tb, aes(k, cor, color = chain)) + geom_line()
```
