...
1. Check if we can model pSTAT, pSTAT3, and pSTAT5 bright cells
2. Check if we can model pSTAT bright, and pSTAT3 and pSTAT5 dim cells
3. Check if we can model zero pERK1_2 and pMAPKAPK bright cells
4. Check if we can model nonzero pERK1_2 and pMAPKAPK bright cells
...
gof = function(df, test_stat) {
  ...
  df_tfm = df %>% mutate_at(obj$protein_names, tfm)
  df_median = df_tfm %>% summarize_at(obj$protein_names, median)
  tibble(
    term = c(
      "1st trimester", 
      "3rd trimester"
...
gof_obsv = bind_rows(
  gof(obj$df_samples_subset, test_stat_a) %>% mutate(subset = "Cell Subset A"),
  gof(obj$df_samples_subset, test_stat_b) %>% mutate(subset = "Cell Subset B"),
  gof(obj$df_samples_subset, test_stat_c) %>% mutate(subset = "Cell Subset C"),
  gof(obj$df_samples_subset, test_stat_d) %>% mutate(subset = "Cell Subset D")
  )
# combined predicted statistic
gof_dist = function(test_stat) {
  stan_pars = rstan::extract(obj$fit_mcmc, pars = "beta")
  mclapply(1:dim(stan_pars$beta)[1], 
           function(k) gof(sample_y_hat(k), test_stat),
           mc.cores = ncores) %>% bind_rows()
}
gof_pred = bind_rows(
  gof_dist(test_stat_a) %>% mutate(subset = "Cell Subset A"),
  gof_dist(test_stat_b) %>% mutate(subset = "Cell Subset B"),
  gof_dist(test_stat_c) %>% mutate(subset = "Cell Subset C"),
  gof_dist(test_stat_d) %>% mutate(subset = "Cell Subset D")
  )
```
...
ggplot(gof_pred, aes(statistic, fill = term)) + 
  geom_histogram(bins = 40, position = "identity", alpha = 0.5) +
  geom_vline(data = gof_obsv, linetype = "dashed", size = 1, 
