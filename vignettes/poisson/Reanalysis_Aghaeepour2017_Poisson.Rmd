...
donor = df_samples_subset$donor %>% as.factor %>% levels
...
subgroups = df_samples_subset %>% 
  group_by(term, donor) %>% 
  tally %>% 
  ungroup
subgroups %<>% mutate(term_index = as.integer(term))
subgroups %<>% mutate(donor_index = as.integer(as.factor(donor)))
# sample one table
...
# sample all tables
sample_info_names = c("donor","term","k")
expr_median = lapply(1:10, 
                     function(i) sample_mu_hat(k = i) %>%
...
# plot MDS
ggmds = ggplot(expr_median, aes_string(x = "MDS1", y = "MDS2",color = color)) +
...
ggmds
ggsave(filename = "posterior_mds_latent_variable_mu.pdf", width = 10, height = 5)
