...
pars = rstan::extract(obj$fit_mcmc, pars = c("Q", "sigma_donor"))
list_pars = lapply(1:nrow(pars$Q), function(i) 
  cor(pars$Q[i,,] %*% diag(pars$sigma_donor[i,]) %*% t(pars$Q[i,,]))
  )
arr_pars = simplify2array(list_pars)
tibble(
  k = 1:dim(arr_pars)[3],
  cor = arr_pars[3,6,]
) %>% ggplot(aes(k, cor)) + 
...
sample_y_hat = function(k = 1) {
  ...
  lapply(1:2, function(cond) {
    ...
    Q = stan_pars$Q[k,,]
    Cov_donor = Q %*% diag(sigma_donor^2) %*% t(Q)
