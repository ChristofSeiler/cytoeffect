...
sample_y_hat = function(k = 1) {
  ...
  lapply(1:2, function(cond) {
    ...
    Q_donor = stan_pars$Q_donor[k,,]
    Cov_donor = Q_donor %*% diag(sigma_donor^2) %*% t(Q_donor)
    ...
    zeros = matrix(rbinom(n_cells_cond*length(obj$protein_names), 
    ...
    count[which(zeros == 1, arr.ind = TRUE)] = 0
    count %<>% as.tibble
    names(count) = obj$protein_names
...
gof_obsv_a = gof(obj$df_samples_subset, test_stat_a)
...
gof_obsv_b = gof(obj$df_samples_subset, test_stat_b)
...
gof_obsv_c = gof(obj$df_samples_subset, test_stat_c)
...
gof_obsv_d = gof(obj$df_samples_subset, test_stat_d)
